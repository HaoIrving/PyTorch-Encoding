...
class BaseDataset(data.Dataset):
    def __init__(self, root, split, mode=None, transform=None, 
                 target_transform=None, base_size=512, crop_size=480):
        self.root = root
    ...
    def _val_sync_transform_sar(self, img, mask):
        outsize = self.crop_size
        short_size = outsize
        # w, h = img.size
        w, h = img.shape[:-1]
        ...
        # img = img.resize((ow, oh), Image.BILINEAR)
        mask = mask.resize((ow, oh), Image.NEAREST)
        img = transform.resize(img, (ow, oh))
        # mask = transform.resize(mask, (ow, oh))
        # center crop
        # w, h = img.size
        w, h = img.shape[:-1]
        x1 = int(round((w - outsize) / 2.))
    ...
    def _sync_transform_sar(self, img, mask):
        # random mirror
        if random.random() < 0.5:
            # img = img.transpose(Image.FLIP_LEFT_RIGHT)
            mask = mask.transpose(Image.FLIP_LEFT_RIGHT)
        ...
        if short_size < crop_size:
            ...
            # img = ImageOps.expand(img, border=(0, 0, padw, padh), fill=0)
            mask = ImageOps.expand(mask, border=(0, 0, padw, padh), fill=0)
            img = self.expand(img, border=(0, 0, padw, padh))
            # mask = self.expand(mask, border=(0, 0, padw, padh))
        # random crop crop_size
        # w, h = img.size
        w, h = img.shape[:-1]
        x1 = random.randint(0, w - crop_size)
    ...
    def expand(self, image, border=None):
        if isinstance(border, tuple):
            left, top, right, bottom = border
        width = left + image.shape[0] + right
        height = top + image.shape[1] + bottom
        if len(image.shape) == 3:
            out = np.zeros((width, height, image.shape[2]), dtype=image.dtype)
        else:
            out = np.zeros((width, height), dtype=image.dtype)
        out[left:image.shape[0],top:image.shape[1]] = image
        return out
